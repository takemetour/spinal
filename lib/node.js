...
Spinal.prototype.job = function(name, data){
  ...
  return {
    ...
    complete: function(complete){ options.complete = complete; return this},
    progress: function(progress){ options.progress = progress; return this},
    failed: function(failed){ options.failed = failed; return this},
    save: function(callback){
      var _callback = function (err, jobId) {
        supportedEvents = ['complete', 'progress', 'failed']
        supportedEvents.forEach(function(event) {
          if (_.isFunction(options[event])) {
            var subKey = 'job:' + jobId + ':' + event
            that.sub.on(subKey, function(err, data) {
              debug(subKey)
              options[event](err, data)
              that.sub.off(subKey)
            })
          }
        })
