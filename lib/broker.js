...
var Broker = function(options){
  ...
  this.pub = new axon.socket('pub-emitter')
  this.pub.server = {
  ...
  this.server.expose('jobAdd', function(name, data, options, reply){
    ...
    if (that.queue){
      console.log('broker: adding job')
      var job = that.queue.addJob(name, data, options, function(err){
        reply(err, job.id)
      }).on('complete', function(data) {
        that.pub.emit('job:' + job.id + ':complete', data)
      }).on('failed', function(err) {
        that.pub.emit('job:' + job.id + ':failed', err)
      }).on('progress', function(progress, data) {
        that.pub.emit('job:' + job.id + ':progress', {progress: progress, data: data})
      })
    } else {
