...
var Broker = function(options){
  ...
  this.server.expose('jobAdd', function(name, data, options, reply){
    ...
    function handleJobEvent(eventName, jobId, arguments) {
      var node = that.router.nodes[data._caller_id]
      if (!node && !node.client) return
      var callArgs = ['job event', eventName, jobId].concat(arguments)
    ...
    if (that.queue){
      ...
      if (data._caller_id) {
        ...
        supportedEvents.forEach(function(eventName) {
          job.on(eventName, function(){
            handleJobEvent(eventName, job.id, [].slice.call(arguments))
          })
        })
      }
...
Broker.prototype.stop = function(done){
  ...
  for (var id in that.router.nodes){
    var loop = function(id){
      that.router.removeNode(id, 'shutdown')
    }(id)
  }
  try {
