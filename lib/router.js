...
Router.prototype.roundrobin = function(method) {
  ...
  return this.routing[method][roundrobin[method]++ % len]
}
...
Router.prototype.call = function(method, arg, options, callback){
  ...
  var call = function(){
    ...
    debug('[rpc] '+method+' @ '+id)
    var stopwatch = that.broker.metrics.timer('methods::' + method).start()
    ...
    that.nodes[id].client.call('rpc', method.split('.')[1], arg, options, function(err, data, _options){
      ...
      callback(err, data, options, _options) // options
    })
  }
