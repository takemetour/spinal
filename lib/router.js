...
var CALL_TIMEOUT = 10000 // default call timeout
...
Router.prototype.roundrobin = function(method) {
  ...
  for (var i = 0; i < len; i++) {
    ...
    if (this.nodes[id].req.connected === true) {
      return id
    }
  }
  // all node is disconnected, return null
  return null
}
...
Router.prototype.removeNode = function(id, reason){
  ...
  node.req.close()
  clearTimeout(node.heartbeatTimeout)
  delete this.nodes[id]
  this.version = crypto.createHash('md5')
    .update(_.keys(this.routing).join(',')).digest('hex')
  debug('[remove] '+node.namespace+' ('+id+') '+reason)
}
...
Router.prototype.call = function(method, arg, options, callback){
  ...
  var call = function(){
    ...
    var id = that.roundrobin(method)
    if (id === null) {
      return callback(
        new Error('Cannot connect any node with method `' + method + '`')
      , null, {}, {})
    }
    ...
    that.nodes[id].client.call('rpc', method.split('.')[1], arg, options, function(err, data, _options){
      ...
      if (_options.cache && that.redis) {
        if (_options.cache.key === true) _options.cache.key = generateHash(arg)
    ...
    timeout = setTimeout(function(){
      ...
      callback(
        new Error('Timeout `router.call('+method+'@'+id+')` exceed ' +
        (options.timeout || CALL_TIMEOUT) + 'ms')
      , null, {}, {})
...
Router.prototype.heartbeating = function(data) {
  ...
  if(node){
    ...
    node.heartbeatTimeout = setTimeout(function(){
      that.removeNode(data.id, 'timeout')
