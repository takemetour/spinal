...
Router.prototype.call = function(method, arg, options, callback){
  ...
  if (this.routing[method]){
    ...
    if(this.routing[method].length > 1) id = random.pick(this.routing[method])
    debug('[rpc] '+method+' @ '+id)
    this.nodes[id].client.call('rpc', method.split('.')[1], arg, options, function(err, options, reply){
      callback(err, options, reply)
    })
  } else {
